...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "-r", "--raw", type=str, default="data/raw/", help="Path to raw dataset.",
    )
    parser.add_argument(
        "-p", "--processed", type=str, default="data/processed/", help="Path to processed dataset.",
    )
...
def stratified_shuffle_split(base_df: pd.DataFrame,) -> tuple[pd.DataFrame, pd.DataFrame]:
    base_df["income_cat"] = pd.cut(
        base_df["median_income"], bins=[0.0, 1.5, 3.0, 4.5, 6.0, np.inf], labels=[1, 2, 3, 4, 5],
    )
...
def pre_process_data(df: pd.DataFrame, imputer: SimpleImputer = None) -> tuple[pd.DataFrame, SimpleImputer]:
