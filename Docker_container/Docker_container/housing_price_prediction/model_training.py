...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "-m", "--models", type=str, default="data/models/", help="Directory to store model pickles.",
    )
...
def run(args: Namespace, logger: Logger) -> None:
    ...
    grid_search = GridSearchCV(
        random_forest, param_grid=param_grid, scoring="neg_mean_squared_error", cv=5, return_train_score=True,
    )
    grid_search.fit(X, y)
